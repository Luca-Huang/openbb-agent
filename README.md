...
1. **多市场买点检测**：A 股 / 港股 / 美股统一 watchlist，触发逻辑基于 20 日突破 + ATR 追高阈值 + 60 日回撤判断，叠加超买闸（距 MA200 ≥ 50% 或 RSI ≥ 85 强制降级），避免暴涨标的被误判为可介入
...
`watchlist_dashboard.py` 把上述信号收敛为一个 verdict 标签放在卡片左条，按由强到弱：

| 标签 | 含义 |
|------|------|
| `建仓` | 突破达标且未越追高线，右侧确认机会 |
| `等回踩` | 已突破但冲高过头，挂回踩单别追 |
| `超买-勿追` | 价格远离中枢 (距 MA200 过远) 或 RSI 极端，无论 trigger 为何都降级 |
| `观察` | 接近突破、结构健康，等放量触发 |
| `回踩-谨慎` | 回踩信号，历史上偏弱，谨慎 |
| `不碰` | 下降趋势无信号，接飞刀区 |

详细规则与改进待办见 `docs/watchlist_dashboard_runbook.md` 和 `docs/dashboard_improvement_backlog.md`。
