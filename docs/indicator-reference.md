...
## 七、个股后续动作建议（Action Advisor）

为每只自选股（无论是否持仓）生成结构化的"下一步该做什么"建议。

**代码位置**：`signal_engine/action_advisor.py`

### 动作分类

| 信号状态 | 动作 | 优先级 | 说明 |
|---|---|---|---|
| `triggered` (breakout) | **可考虑入场（突破）** | 1 — 紧急 | 放量突破 20 日新高，生成完整入场计划 |
| `triggered` (pullback) | **可考虑入场（回踩）** | 1 — 紧急 | 回踩 MA50 且结构未破，止损空间通常较小 |
| `triggered` + 高事件风险 | **可考虑入场（等事件）** | 2 — 重要 | 有触发但近期有重大事件，建议等落地再决定 |
| `near_zone` | **准备入场计划** | 2 — 重要 | 距目标区间 ≤ 8%，制定止损/仓位并设置提醒 |
| `watch` (conviction ≥ 40) | **重点跟踪** | 3 — 一般 | 基本面有吸引力但技术面未配合 |
| `watch` (conviction 20~40) | **常规观察** | 4 — 低 | 条件未成熟，每周复查 |
| `watch` (conviction < 20) | **低优先级** | 4 — 低 | 无催化剂，保留但无需频繁关注 |
| `invalidated` | **暂停关注** | 3 — 一般 | 结构被破坏，等待价格重新站上关键均线 |

### 入场计划（triggered 状态自动生成）

当信号处于 `triggered` 状态时，系统自动计算并输出：

```
入场参考: {当前收盘价}
止损位:   {支撑位 / MA50 / MA200 中最高者}
止盈1:    {入场 + 1R}
止盈2:    {入场 + 2R}
仓位建议: 单笔风险 X%，用总资金的 1-2% 作为最大亏损额反算仓位
```

### 优先级排序

动作列表按优先级升序排列（1 最高），便于在每日复查时先处理最紧急的股票。
